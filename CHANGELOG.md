...
- `peers.json` schema gains optional `identity: { oracle, node }` field captured from each peer's `/api/identity` response. `maw doctor` now flags duplicate `<oracle>:<node>` claims across the cache (new `peers:duplicates` check), and `maw serve` warns at boot when the local identity collides with any cached peer or when two peers claim the same `<oracle>:<node>` pair. Per ADR `docs/federation/0001-peer-identity.md` ("Crypto solves can't-fake; doctor + boot-time check solves operator confusion"): non-blocking — operator decides. `/api/identity` now also advertises `oracle` (default `"mawjs"`) so peers can capture it. Step 3 of #804.
