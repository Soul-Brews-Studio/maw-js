...
function taskCard(task, opts) {
  ...
  // kobo-255: the wait-label belongs to the blocked lane only. slice-A makes a
...
// kobo-396 — escapeHtml/inlineMd/mdToHtml moved to src/views/md.ts (shared with
// room.ts, was a duplicate-in-waiting per the 2-live-path lesson). Injected here
// verbatim via toString() — same single-source pattern as stateBadge etc. above.
${escapeHtml.toString()}
${inlineMd.toString()}
${mdToHtml.toString()}
