...
export interface PluginDownloadApiDeps {
  discoverPackages?: typeof discoverPackages;
  lstatSync?: typeof lstatSync;
  spawn?: typeof spawn;
}

export interface PluginDownloadChild {
  stdout: {
...
export function createPluginTarStream(child: PluginDownloadChild): ReadableStream<Uint8Array> {
...
export function createPluginDownloadApi(deps: PluginDownloadApiDeps = {}) {
  const discover = deps.discoverPackages ?? discoverPackages;
  const lstat = deps.lstatSync ?? lstatSync;
  const spawnTar = deps.spawn ?? spawn;

  return new Elysia().get(
    "/plugin/download/:name",
    ({ params, set }) => {
      const name = params.name;
      const entry = discover().find(p => p.manifest.name === name);
      if (!entry) {
        set.status = 404;
      ...
      let isSymlink = false;
      try {
        isSymlink = lstat(entry.dir).isSymbolicLink();
      } catch {
      ...
      const child = spawnTar("tar", ["-czf", "-", "-C", entry.dir, "."]);
      return createPluginTarStream(child);
    },
  );
}

export const pluginDownloadApi = createPluginDownloadApi();
