...
export async function cmdTmuxPeek(target: string, opts: TmuxPeekOpts = {}): Promise<void> {
  ...
  try {
    ...
    throw new Error(`tmux capture-pane failed for '${resolved}' (from ${source}): ${e instanceof Error ? e.message : String(e)}`);
  }
...
export async function cmdTmuxSend(target: string, command: string, opts: TmuxSendOpts = {}): Promise<void> {
  ...
  try {
    ...
    throw new Error(`pane lookup failed for '${resolved}' (from ${source}): ${e instanceof Error ? e.message : String(e)}`);
  }
  ...
  try {
    ...
    throw new Error(`send-keys failed for '${resolved}': ${e instanceof Error ? e.message : String(e)}`);
  }
...
export async function cmdTmuxSplit(target: string, opts: TmuxSplitOpts = {}): Promise<void> {
  ...
  try {
    ...
    throw new Error(`split-window failed for '${resolved}' (from ${source}): ${e instanceof Error ? e.message : String(e)}`);
  }
...
export async function cmdTmuxKill(target: string, opts: TmuxKillOpts = {}): Promise<void> {
  ...
  try {
    ...
    throw new Error(`kill failed for '${resolved}' (from ${source}): ${e instanceof Error ? e.message : String(e)}`);
  }
...
export async function cmdTmuxLayout(target: string, preset: string): Promise<void> {
  if (!(VALID_LAYOUTS as readonly string[]).includes(preset)) {
  ...
  try {
    ...
    throw new Error(`select-layout failed for '${window}' (from ${source}): ${e instanceof Error ? e.message : String(e)}`);
  }
