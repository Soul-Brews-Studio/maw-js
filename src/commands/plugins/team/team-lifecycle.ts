...
export async function cmdTeamSpawn(
  ...
  if (existsSync(toolConfigPath)) {
    try {
      ...
      if (!toolConfig.members.some((m: { name: string }) => m.name === role)) {
  ...
  const teammateCount = manifest.members.filter((m: { name: string }) => m.name !== role).length;
  ...
  if (opts.exec) {
    ...
    try {
      ...
      if (existsSync(toolConfigPath)) {
        try {
          const toolConfig = JSON.parse(readFileSync(toolConfigPath, "utf-8"));
          const member = toolConfig.members?.find((m: { name: string }) => m.name === role);
          if (member) {
