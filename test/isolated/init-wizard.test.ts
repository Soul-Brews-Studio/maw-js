/**
 * init-wizard.test.ts — subprocess tests for `maw init --non-interactive` (#455).
 *
 ...
 *
 * This file lives in test/isolated/ so each test-file gets its own bun
 * process (#429 per-file subprocess). Inside the file, each `test()` mkdtemps
 * its own env, so cases don't leak into each other.
 *
 * The tests target the CLI contract defined in the init-wizard spec § 4c.
 * Written BEFORE the wizard ships (PR #455); will FAIL until impl lands.
 */
