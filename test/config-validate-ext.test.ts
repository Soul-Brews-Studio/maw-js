...
describe("validateConfig extended fields", () => {
  test("accepts and trims every supported extended config field", () => {
    ...
    const { result, warnings } = validateWithWarnings({
      ...
      broadcastTo: ["nanoclaw", "lora-mesh"],
      errorForward: { target: "  doctor-alpha  " },
  ...
  test("filters invalid array entries while warning for lossy containers", () => {
    ...
    const { result, warnings } = validateWithWarnings({
      ...
      broadcastTo: ["nanoclaw", 17, "lora-mesh"],
      errorForward: { target: "   " },
    ...
    expect(warnings).toEqual([
      ...
      "[maw] config warning: errorForward.target must be a non-empty string, using default",
  ...
  test("warns for invalid extended-field container types", () => {
    ...
    expect(warnings).toEqual([
      ...
      "[maw] config warning: broadcastTo must be an array of transport names, using default",
      "[maw] config warning: errorForward must be an object, using default",
