...
describe("engine plugin registry (#1566)", () => {
  ...
  test("covers validation edge cases and unregister variants", () => {
    expect(() => registerEnginePlugin({ plugin: "bad", prefix: "/api/", upstream: "http://127.0.0.1:1" }))
      .toThrow(/include a plugin path/);
    expect(() => registerEnginePlugin({ plugin: "bad", prefix: "/api/bad path", upstream: "http://127.0.0.1:1" }))
      .toThrow(/clean absolute/);
    expect(() => registerEnginePlugin({ plugin: "bad", prefix: "/api/bad", upstream: "not a url" }))
      .toThrow(/must be a URL/);
    expect(() => registerEnginePlugin({ plugin: "bad", prefix: "/api/bad", upstream: "https://127.0.0.1:1" }))
      .toThrow(/loopback http/);
    expect(() => registerEnginePlugin({ plugin: "bad", prefix: "/api/bad", upstream: "unix:tmp/plugin.sock" }))
      .toThrow(/absolute socket/);
    expect(() => registerEnginePlugin({ plugin: "bad", prefix: "/api/bad", upstream: "unix:///%00.sock" }))
      .toThrow(/null bytes|whitespace/);
    expect(() => registerEnginePlugin({ plugin: "bad", prefix: "/api/bad", upstream: "unix:///tmp/a%20b.sock" }))
      .toThrow(/null bytes|whitespace/);
    expect(() => registerEnginePlugin({ plugin: "bad", prefix: "/api/bad", upstream: "unix:///tmp//plugin.sock" }))
      .toThrow(/traversal/);
    expect(() => registerEnginePlugin({ plugin: "bad", prefix: "/api/bad", upstream: "unix:///tmp/plugin.sock?x=1" }))
      .toThrow(/no host, query, or hash/);
    expect(() => registerEnginePlugin({ plugin: "bad", prefix: "/api/bad", upstream: "http://127.0.0.1:1", events: ["ok", " "] }))
      .toThrow(/events/);
    expect(() => registerEnginePlugin({ plugin: "bad", prefix: "/api/bad", upstream: "http://127.0.0.1:1", eventPath: "relative" }))
      .toThrow(/eventPath/);
    expect(() => registerEnginePlugin({ plugin: "bad", prefix: "/api/bad", upstream: "http://127.0.0.1:1", health: "/bad path" }))
      .toThrow(/health/);

    registerEnginePlugin({ plugin: "replace-me", prefix: "/api/replace-me", upstream: "http://127.0.0.1:1" });
    registerEnginePlugin({ plugin: "replace-me", prefix: "/api/replace-me-next", upstream: "http://127.0.0.1:2" });
    expect(findEnginePluginRegistration("/api/replace-me/old")).toBeUndefined();
    expect(findEnginePluginRegistration("/api/replace-me-next/new")?.plugin).toBe("replace-me");

    expect(unregisterEnginePlugin({ prefix: "/api/replace-me-next" })).toBe(true);
    expect(unregisterEnginePlugin({ prefix: "/api/replace-me-next" })).toBe(false);
    registerEnginePlugin({ plugin: "remove-plugin", prefix: "/api/remove-plugin", upstream: "http://127.0.0.1:3" });
    expect(unregisterEnginePlugin({ plugin: "remove-plugin" })).toBe(true);
    expect(unregisterEnginePlugin({ plugin: "missing-plugin" })).toBe(false);
    expect(() => unregisterEnginePlugin({ plugin: "Bad_Plugin" })).toThrow(/plugin/);
  });

  test("health helpers cover no-health and polling warning paths", async () => {
    const healthy = registerEnginePlugin({
      plugin: "no-health",
      prefix: "/api/no-health",
      upstream: "http://127.0.0.1:1",
    });
    expect(await pollEnginePluginHealth()).toEqual({ checked: 0, removed: 0 });
    expect(findEnginePluginRegistration("/api/no-health/ping")).toBe(healthy);

    const warned: string[] = [];
    const oldWarn = console.warn;
    console.warn = (message?: unknown) => { warned.push(String(message)); };
    const stop = startEnginePluginHealthPolling(1, async () => { throw new Error("poll boom"); });
    try {
      await Bun.sleep(10);
    } finally {
      stop();
    ...
    expect(warned.some((line) => line.includes("health poll failed") && line.includes("poll boom"))).toBe(true);
  });

  test("event delivery records non-ok responses without unbinding", async () => {
    const upstream = Bun.serve({
      port: 0,
      hostname: "127.0.0.1",
      fetch: () => Response.json({ ok: false }, { status: 503 }),
    });
    try {
      registerEnginePlugin({
        plugin: "event-503",
        prefix: "/api/event-503",
        upstream: `http://127.0.0.1:${upstream.port}`,
        events: ["MessageSend"],
      });
      const result = await dispatchEnginePluginEvent({
      ...
      expect(result).toEqual({ delivered: 0, failed: 1, removed: 0 });
      expect(findEnginePluginRegistration("/api/event-503/messages")?.plugin).toBe("event-503");
    } finally {
      upstream.stop(true);
    }
  });
